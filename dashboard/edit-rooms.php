...
<html lang="en">
    <head>
        <meta charset="UTF-8" />
        <title>TNBooking - Edit Room</title>
    ...
    <body>
            ...
            <div class="row">
                ...
                <div class="main-content p-0 overflow-hidden">
                    ...
                    <div class="card m-5 p-4">
                        <h4 id="pageTitle">Edit Room</h4>
                        <form id="roomForm" novalidate>
                            <input type="hidden" id="roomId" name="roomId" value="<?= $_GET['id'] ?>"/>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" id="guestsAllowed" name="guestsAllowed" class="form-control" min="1" required />
                                    <div class="invalid-feedback">Please provide the number of guests allowed.</div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="roomDescription" name="roomDescription" class="form-control" rows="4" required></textarea>
        ...
        <script>
            $(document).ready(function () {
                ...
                if (roomId) {
                    $("#roomId").val(roomId);
                    $("#pageTitle").text("Edit Room");
                    loadRoomData(roomId);
                } else {
                    $("#pageTitle").text("Add New Room");
                }

                // --------- Load Existing Data (for Edit) ---------
                function loadRoomData(roomId) {
                    $.getJSON("../api/hotel/get-room?id=" + roomId, function (data) {
                        // Check if data is an array and get the first item
                        const roomData = Array.isArray(data) && data.length > 0 ? data[0] : data;
                        
                        if (!roomData) {
                            showToast("Error", "Failed to load room data.", "error");
                            return;
                        }
                        
                        $("#roomType").val(roomData.room_type);
                ...
                $("#roomForm").on("submit", function (e) {
                    ...
                    $("#roomType, #guestsAllowed, #roomDescription, #pricePerNight").each(function () {
                        if (!$(this).val().trim()) {
                ...
                function submitForm() {
                    ...
                    formData.append("roomType", $("#roomType").val().trim());
